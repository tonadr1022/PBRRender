...
glm::mat4 OrbitCamera::GetProjection(float aspect_ratio, float fov_degrees) const {
  return glm::perspective(glm::radians(fov_degrees), aspect_ratio, near_plane_, far_plane_);
}

glm::mat4 OrbitCamera::GetView() const { return glm::lookAt(pos_, target_, kUpVector); }
...
void OrbitCamera::Update(Window&) {}
